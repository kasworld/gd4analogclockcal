...
func init(r:float, numrate:float = 0.9, ftrate :float = 0.09,numoutline :int = 4, lntk :float =0.004,  dnm :DrawNumMode = DrawNumMode.Hour)->void:
	main_radius = r
	...
	match draw_num_mode:
		DrawNumMode.Hour:
			for i in range(1,13):
				...
				draw_letter(rad,nums_radius_rate *main_radius, main_radius*font_size_rate, i)
		DrawNumMode.Minute:
